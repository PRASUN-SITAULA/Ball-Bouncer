...
float theta = pi<float>() / 2.f, phi = 0;
float radius = 9;
...
std::vector<vec3> translation(3);
bool lmouse_button = false;
bool shift_button = false;

bool rmouse_button = false;
glm::vec3 ray_world = {};
int zoom = 0;
bool scroll = false;

float xmax = 5.1f, xmin = -5.1f, ymax = 5.1f, ymin = -5.1f, umax = 5.1f, umin = -5.1f, vmax = 5.1f, vmin = -5.1f;
...
float normalize(float value, float min_value, float max_value) {
	return (value - min_value) / (max_value - min_value) * 5.1f;
}
...
void computeInputs() {
	...
	if (lmouse_button) {
		...
		if (!shift_button) {
			...
			if (verticalAngle > -1 * pi<float>() / 2.f && verticalAngle < pi<float>() / 2.f) {
				phi = verticalAngle;
			}
	...
	if (rmouse_button) {
		// Get mouse position
		double xpos, ypos, zpos = 501.f;
		glfwGetCursorPos(window, &xpos, &ypos);

		//std::cout << xpos << "\t" << ypos << std::endl;

		glm::mat4 coordinate_matrix = glm::mat4(
			glm::vec4(xpos, 0.0, 0.0, 0.0),
			glm::vec4(ypos, 0.0, 0.0, 0.0),
			glm::vec4(zpos, 0.0, 0.0, 0.0),
			glm::vec4(1.0, 0.0, 0.0, 0.0)
		);

		glm::mat4 translation_matrix = glm::translate(glm::mat4(1.0), glm::vec3(-500, -500, -500));
		glm::mat4 rotation_matrix_xz = glm::mat4(
			glm::vec4(cos(theta), 0.0, sin(theta), 0.0),
			glm::vec4(0.0, 1.0, 0.0, 0.0),
			glm::vec4(-sin(theta), 0.0, cos(theta), 0.0),
			glm::vec4(0.0, 0.0, 0.0, 1.0)
		);
		glm::mat4 rotation_matrix_yz = glm::mat4(
			glm::vec4(1.0, 0.0, 0.0, 0.0),
			glm::vec4(0.0, cos(pi<float>() / 2 - phi), -sin(pi<float>() / 2 - phi), 0.0),
			glm::vec4(0.0, sin(pi<float>() / 2 - phi), cos(pi<float>() / 2 - phi), 0.0),
			glm::vec4(0.0, 0.0, 0.0, 1.0)
		);
		glm::mat4 translation_matrix_back = glm::translate(glm::mat4(1.0), glm::vec3(500, 500, 500));

		glm::mat4 final_matrix = translation_matrix_back * rotation_matrix_yz * rotation_matrix_xz * translation_matrix * coordinate_matrix;
		...
		xpos = final_matrix[0][1];
		ypos = final_matrix[1][1];
		zpos = final_matrix[2][1];

		std::cout << xpos << "\t" << ypos << "\t" << zpos << std::endl;

		float ndcX = (float)xpos - WIDTH / 2;
		float ndcY = (float)ypos - HEIGHT / 2;
		float ndcZ = (float)zpos - 500;

		std::cout << ndcX << "\t" << ndcY << "\t" << ndcZ << std::endl;

		//ray_world = { normalize(ray_world.x, 0, 500), -1 * normalize(ray_world.y, 0, 500), 1.f };

		ray_world = { normalize(ndcX, 0, 500), -1 * normalize(ndcY, 0, 500), normalize(ndcZ, 0, 500) };// 1.f };

		ray_world.x = ((xmax + Sf) - (xmin - Sf)) * (ray_world.x - umin) / (umax - umin) + (xmin - Sf);
		ray_world.y = ((ymax + Sf) - (ymin - Sf)) * (ray_world.y - vmin) / (vmax - vmin) + (ymin - Sf);

		translation[1] = ray_world;
		height = ray_world.y;
		velocity = INIT_VELOCITY;

		// Output the world-space coordinates of the mouse cursor
		//std::cout << "Mouse World Position: " << ray_world.x << ", " << ray_world.y << ", " << ray_world.z << std::endl;
	}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
	...
	if (yoffset > 0) {
		// Middle mouse scroll forward action
		// Perform actions for scrolling forward
		scroll = true;
		zoom = 1;
		std::cout << "Forward\n";
		...
		// Middle mouse scroll backward action
		// Perform actions for scrolling backward
		scroll = true;
		zoom = -1;
	}
}
...
void initPosition() {
	...
	translation[1] = ray_world;// vec3(0.0f, 0.5f, 0.0f) + vec3(0.0f, height, 0.0f);
}

void updatePosition(float dtime) {
	float distance = abs(translation[1].y - translation[0].y);

	if (distance <= 0.5 && frames_rendered > 1) {
...
int main(void)
{
	...
	int spherestart = vertices.size();//planesize;
	...
	do {
...

		{
			...
			glm::mat4 RotationMatrix = rotate(mat4(1.0f), pi<float>() / 2.f, vec3(0, 1, 0));
			glm::mat4 TranslationMatrix = translate(mat4(1.0f), translation[0]); // A bit to the right
			glm::mat4 ScalingMatrix = scale(mat4(1.0f), vec3(5.0f, 5.0f, 5.0f));
		...
		// the ball
		{
			...
			glm::mat4 RotationMatrix = rotate(mat4(1.0f), pi<float>() / 2.f, vec3(0, 1, 0));
			vec3 position = translation[1];//translation[0] + translation[1];
